...
def buscar_por_id(unidade_id: int, db: Session = Depends(get_db)):  # noqa: B008
    return crud.buscar_consulta_unidade(db, unidade_id)

@router.get("/intervalo", response_model=list[schemas.ConsultaResponse])
def buscar_intervalo(hora_inicio: time, hora_fim: time, db: Session = Depends(get_db)):  # noqa: B008
